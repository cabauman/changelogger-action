...
export const SUPPORTED_BRANCH_COMPARISON_STRATEGIES = [
  'tag',
  'workflow',
] as const
export type BranchComparisonStrategy =
  typeof SUPPORTED_BRANCH_COMPARISON_STRATEGIES[number]
...
export default function retrieveAndValidateInput(
  ...
  const branchComparisonStrategy = validateBranchComparisonStrategy(
    rawBranchComparisonStrategy,
  )
...
function validateBranchComparisonStrategy(
  value: string,
): BranchComparisonStrategy {
  if (
    !SUPPORTED_BRANCH_COMPARISON_STRATEGIES.includes(
      value as BranchComparisonStrategy,
    )
  ) {
    throw new Error(
